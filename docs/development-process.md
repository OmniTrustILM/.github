...
| 10 | Manual dispatch | `.github` repo's `repo-template-sync.yml` | Aligns every target repo with the org templates (release.yml + caller workflows + CODEOWNERS, plus Copilot review instructions for repos in `config/copilot-repos.yml`) in one PR per repo (up to 4 commits, skipped if no drift). CODEOWNERS is rendered per repo from `config/repo-domains.yml`. |
...
**Propagation to target repos:** changes to `templates/labels.yml` propagate automatically via `label-sync.yml` on push to main. Changes to `templates/release.yml` and `templates/caller-workflows/*.yml`, the per-repo `.github/CODEOWNERS` rendered from `config/repo-domains.yml`, and `templates/copilot-instructions.md` for repos listed in `config/copilot-repos.yml`, are distributed by manual dispatch of `repo-template-sync.yml` (opens one PR per target repo with up to 4 commits; skips repos with no drift). For release.yml-only hotfixes, `release-yml-sync.yml` dispatches a focused rollout.
...
| `/epic-breakdown` | Available | Read an Epic (or a requirement), explore repos and existing issues, generate Acceptance Criteria + Technical Analysis + Impact Assessment + Testing Scope, and propose sub-issues by work stream with Module/Complexity set, dependencies, and a seed Estimate the Developer owns (an override is final) — all behind a human-approval gate. Adds a *preflight* mode (analysis + blocking questions when the design is undecided) and a *reconcile* mode (sync an Epic with current progress). Sanctioned writer of Complexity/Estimate (§3.1, §3.5). |
