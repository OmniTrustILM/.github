...
- **Test phase labels are house style (Check 3)** — `// given`, `// when`, `// then` and `// and` labels in a test body are *mandated* by the org's `align-test-style` conventions ("every test body has `// given`, `// when`, `// then` blocks separated by blank lines"), so neither a bare label nor one carrying a short scenario note (`// given — no secret with this name exists`) is ever a restatement finding. Flagging them would put the author between two org skills that contradict each other, and `apply all` would strip the structure the other one requires.
