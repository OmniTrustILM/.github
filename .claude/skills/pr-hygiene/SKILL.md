...
- **Test phase labels are house style (Check 3)** — `// given`, `// when`, `// then` and `// and` labels in a test body are house style, so neither a bare label nor one carrying a short scenario note (`// given — no secret with this name exists`) is a restatement finding.
