...
#
# Both modes report parent_writable.
#
...
# true / false / unknown. Only an explicit false is worth warning about, so an
# explicit false must survive — `// "unknown"` would swallow it alongside null.
parent_writable() {
  local repo="$1" perms
  perms=$(gh api "repos/$ORG/$repo" \
    --jq '.permissions.push | if type == "boolean" then tostring else "unknown" end' \
...
if [ "$MODE" = "cycles" ]; then
  ...
  while :; do
    ...
    CYCLES=$(jq -n --argjson acc "$CYCLES" --argjson page "$PAGE" \
      '$acc + [ $page.data.repository.issues.nodes[]
                | select(.title | test("^Bugs [0-9]+\\.[0-9]+\\.[0-9]+$"))
                | { version: (.title | ltrimstr("Bugs ")), number, node_id: .id, sub_issues: .subIssues.totalCount } ]')
